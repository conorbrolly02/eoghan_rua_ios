...
def _validate(players: List[Player], n_teams: int) -> None:
    ...
    for name, skill in players:
        if not isinstance(skill, int) or skill not in (1, 2, 3):
            raise ValueError(f"Invalid skill for player '{name}': {skill} (must be 1, 2, or 3)")
        if not str(name).strip():
            raise ValueError("Player name cannot be empty.")

def _group_by_tier(players: List[Player]) -> Dict[Skill, List[str]]:
    tiers: Dict[Skill, List[str]] = {1: [], 2: [], 3: []}
    for name, skill in players:
...
def pick_teams(players: List[Player], n_teams: int, seed: Optional[int] = None) -> List[Dict]:
    ...
    for i in range(n_teams):
        teams.append({"name": f"Team {i+1}", "players": [], "counts": {1:0,2:0,3:0}, "score": 0})
    ...
    for skill in (1, 2, 3):
        ...
        if leftover > 0:
            def priority_iter():
                ...
                rank = {team_idx: pos for pos, team_idx in enumerate(order)}
        ...
        for team_idx in order:
            ...
            for _ in range(take):
                if idx >= total: break
                nm = names[idx]
                teams[team_idx]["players"].append((nm, skill))
                teams[team_idx]["counts"][skill] += 1
                teams[team_idx]["score"] += WEIGHTS[skill]
                idx += 1

    # final balance pass (lowest tier moves)
    ...
    while changed:
        changed = False
        target = target_sizes
        over = [i for i in range(n_teams) if len(teams[i]["players"]) > target[i]]
        under = [i for i in range(n_teams) if len(teams[i]["players"]) < target[i]]
        if not over or not under: break
        over.sort(key=lambda i: len(teams[i]["players"]) - target[i], reverse=True)
        under.sort(key=lambda i: target[i] - len(teams[i]["players"]), reverse=True)
        src, dst = over[0], under[0]
        mv = pop_lowest(teams[src])
        if mv:
            nm, s = mv
            teams[dst]["players"].append((nm,s))
            teams[dst]["counts"][s] += 1
            teams[dst]["score"] += WEIGHTS[s]
            changed = True
